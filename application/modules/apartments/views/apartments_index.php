...
<input type="text" id="datepicker" class="form-control" name="from_apt" value="<?php if(!empty($date)){ echo $date; }else{  } ?>" placeholder="Check-In-Date" autocomplete="off">
...
<div class="styled-select-small">
<div class="position-absolute"><i class="fa fa-calendar"></i></div>
<input type="text" id="datepicker1" class="form-control" name="to_apt" value="<?php if(!empty($to_date)){ echo $to_date; }else{  } ?>" placeholder="Check-Out-Date" autocomplete="off">
...
<select class="form-control" id="child" name="child_apt">
...
<div class="styled-select-small">
<div class="position-absolute"><i class="fa fa-user"></i></div>
<select class="form-control" id="adult" name="adult_apt">
...
<div class="styled-select-small">
<div class="position-absolute"><i class="fa fa-user"></i></div>
<select class="form-control" id="room" name="room_apt">
...
<button type="submit"  name="search" class="btn btn-primary btn-search search-transfer  btn-block" id="findapt"> <i class="fa fa-search"></i> Find Apartments</button>
</div>
</div>
</div>
...
<p>Category</p>
 <div class="custom-control custom-checkbox">
  <input type="checkbox" class="custom-control-input" name="starcheck" value="5" id="customCheck5" >
...
<div class="custom-control custom-checkbox">
  <input type="checkbox" class="custom-control-input" name="starcheck" value="4" id="customCheck4">
...
<div class="custom-control custom-checkbox">
  <input type="checkbox" class="custom-control-input" name="starcheck" value="3" id="customCheck3">
...
<div class="custom-control custom-checkbox">
  <input type="checkbox" class="custom-control-input" name="starcheck" value="2" id="customCheck2">
...
<div class="col-sm-9">
 <div class="row">
<div class="col">
 <button class="btn btn-sm btn-outline-primary" id="rowcount"><?php   echo $numrow;  ?> Apartments Found in Penang</button>
</div>
...
<a class="btn btn-sm btn-outline-secondary column_sorthotel" id="htlname"  hn="asc"  >Apartments Name</a>
...
<img class="img-fluid" src="<?php echo base_url(); ?>uploads/apartmentmaster/<?php echo $val['apt_pic']; ?>"/>	
</a>
<div class="cbp-vm-details">
	<h5 class="project-name text-uppercase"><?php echo $val['aptname'];  ?>
 ...
 <div class="call-back">
	...
	<h3 class="price1" data-toggle="popover" data-trigger="hover" data-popover-content="#a1<?php echo $val['id']; ?>" data-placement="top"> MYR <?php echo $val['apt_price']; ?>  
	</h3>
	 ...
	 <div class="collapse clearfix" data-target='t2<?php echo $val['id']; ?>'>
		<div class="">
			<div class="flexslider">
			 <ul class="slides">
				 <?php
					  $hotelGallery = getaptgallery($val['id']);
...
<?php	}else{ ?>
    ...
    <?php if($numrow>0){ ?>
	<button type="button" name="search" class="btn btn-primary btn-search search-transfer  btn-block" id="loadapt"> Load More</button>	
	<?php } ?>	
</div>
</div>
</div>
  ...
  function load_data(search,order,htlorder,starrate,catstar)
   {
    ...
    $.ajax({
    url:"<?php echo base_url(); ?>apartments/searchhtlname",
    method:"POST",
    data:{query:search,order:order,date:date,htlorder:htlorder,starrate:starrate,catstar:catstar,offset:offset,limit:limit},
	beforeSend:function(){
		$.preloader.start({
			modal: true,
			position: 'center'
  ...
  var search = $(this).val();
   ...
   $(document).on('click', '.column_sortprice', function(){ 
	...
	$(document).on('click', '.custom-control-input', function(){ 
			...
			$('input[name="starcheck"]:checked').each(function() {
			console.log($('input[name="starcheck"]:checked').serialize());
     });	   
	});
