...
#results {
    ...
    <section id="main-content">
        <section class="wrapper">
            <h3><i class="fa fa-angle-right"></i> Event Management</h3>
            <?php
				      ...
				      foreach($pages as $row){ ?>
                        <tr>
                            ...
                            <td data-title="Event Logo">
                                ...
                                ?> 
                                <a href="<?php echo base_url() ."uploads/event/".$row['logo']."";  ?> " target="_blank">
                                <?php echo "<img src='".base_url()."/uploads/event/".$row['logo']."' width='100px;' height='71px;'>";
                                ?></a>
                                 <?php } ?>
                            </td>
                            ...
                            <td data-title="Background Image">
                                <?php 
                                if(!empty($pages[0]['bg_color'])) {
                                ?>
                                <a href="<?php echo base_url() ."uploads/event/".$row['bg_color']."";  ?> " target="_blank">
                                <?php echo "<img src='".base_url()."/uploads/event/".$row['bg_color']."' width='100px;' height='71px;'>";
                                 ?></a>
                                <?php } ?>
                            </td>

                            <td data-title="Event Description">
                                <?php
                            ...
                            <td data-title="Action">
                                <a class="btn btn-default btn-xs"  href="<?php echo SUPPLIER_URL. "event/view/".$row['id']."/".($page-1);?>" title="View" ><i class="fa fa-eye"></i></a>
                                    <a class="btn btn-primary btn-xs" href="<?php echo base_url() ."supplier/event/edit/".$row['id']; ?>" onclick="return confirm('Are you sure you want to edit <?php echo $row['id'] ; ?>?');" title="Edit"><i class="fa fa-pencil"></i></a>
                                                            ...
                                                            <!---status----->
                                    <?php  if($row['status']==1){?>
                                    <a href="<?php echo base_url() ."supplier/event/status/".$row['id']."/0 ";?>" class="btn btn-success btn-xs" title="Enable" onclick="return confirm('Are you sure you want to disable <?php $row['id'] ; ?>?');"> <i class="fa fa-check"></i></a>
                                <?php } else { ?>
                                 <a class="btn btn-warning btn-xs" href="<?php echo base_url() ."supplier/event/status/".$row['id']."/1 ";?>" title="Disable" onclick="return confirm('Are you sure you want to enable <?php echo $row['id'] ; ?>?');"> <i class="fa fa-check"></i></a>
                                <?php } ?>
                                                                        <!----end status------>
                                 <!----delete------>
