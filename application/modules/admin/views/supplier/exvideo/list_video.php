			...
			?>
              ...
              <div class="col-lg-12">
				...
				<section id="no-more-tables">
				 ...
				 <?php 				
				  if(!empty($records)){
					$viewd_item = $this->session->userdata('v_item');
					if(!empty($viewd_item)){
					 $i= $viewd_item;	
					}
					else {
				  ...
				  ?>
                   ...
                   <tbody>
					  ...
					  if(!empty($row->snippet->resourceId->videoId)){
						 $video_id = $row->snippet->resourceId->videoId;
						  /**Get video tags**/
						/*  $key_url = 'http://www.youtube.com/watch?v='.$video_id;
					...
					if(!empty($tags)){
					  ...
					  ?>
						...
						<td data-title="Download"><input type="checkbox" name="arr_vid[]" value="<?php echo $video_id;?>" class="down_ck"></td>
						  <td data-title="Title">
...
<iframe width="200" height="100" src="https://www.youtube.com/embed/<?php echo $video_id;?>" frameborder="0" allowfullscreen></iframe>
					    </td>
							   
				
			  
				<td data-title="Download" id="dwn-<?php echo $video_id;?>">
				<a class="btn btn-primary btn-xs download" vid="<?php echo $video_id;?>" cid="<?php echo $row->snippet->channelId;?>" desc="<?php echo $row->snippet->description;?>"  href="javascript:void(0);" title="Download"><i class="fa fa-download"></i></a>
				</td>
						  </tr>
				      <?php 
					  $i++; 
	   ...
	   if(!empty($prevPageToken)){
		   $link_nav = site_url().'supplier/exvideo/view?pid='.$playlist_id.'&prev='.$prevPageToken;
		   echo "<a href='".$link_nav."'>Prev</a>";
	   }
	   if(!empty($nextPageToken)){
		   $link_nav = site_url().'supplier/exvideo/view?pid='.$playlist_id.'&next='.$nextPageToken;
		   echo "<a href='".$link_nav."'>Next</a>";
	   }
	   
	   ?>
	   </div></td>
	   <input type="hidden" name="channel_id" value="<?php echo $this->session->userdata('channel_id'); ?>">
        </tr>
