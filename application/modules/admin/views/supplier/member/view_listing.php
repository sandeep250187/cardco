      ...
      <section id="main-content">
		  ...
		  ?>
          	<h3><i class="fa fa-angle-right"></i> Dentist Management <div align="right">
							  <a href="<?php echo SUPPLIER_URL; ?>/importcsv" class="btn btn-theme custom_blue_btn">Import Dentist</a>
							  <a href="<?php echo SUPPLIER_URL; ?>importcsv/download/<?=$name;?>/<?=$email;?>/<?=$from;?>/<?=$to;?>" class="btn btn-theme custom_blue_btn">Export Dentist</a>
							  </div></h3>
		<?php 
	 ...
	 //}
		...
		<a class="btn btn-warning btn-xs" href="<?php echo SUPPLIER_URL. "member/status/".$row['id']."/1/".($page-1);?>"  title="Disable" onclick="return confirm('Are you sure you want to enable <?php echo $row['fname'].' '.$row['lname']; ?>?');"> 
			...
			$deleteUrl =  (empty($row['lock_row']))?(SUPPLIER_URL. "member/delete/".$row['id']."/".($page-1)):"javascript:alert('Remove Not Allowed For ".$row['email'].")"; ?>
						...
						<a class="btn btn-plus btn-xs btn-delivery" href="<?php echo SUPPLIER_URL. "member/deliveryinfo/".$row['id'];?>" title="Update Delivery Info"><i class="fa fa-plus"></i></a>
						  
						<a class="btn btn-plus btn-xs btn-credit" href="<?php echo SUPPLIER_URL. "member/creditdetails/".$row['id'];?>" title="Add Credit"><i class="fa fa-plus"></i></a>
			<?php  } 
			?>
