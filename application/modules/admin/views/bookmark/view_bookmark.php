              ...
              <div class="col-lg-12">
				...
				<?php $i=++$page;?>
				      ...
				      foreach($bookmarks as $row){ ?>
						...
						<td data-title="Email">
						<!---edit page---->
						<a class="btn btn-primary btn-xs"  href="<?php echo base_url() . "admin/bookmark/edit/".$row['ID']; ?>" onclick="return confirm('Are you sure you want to edit <?php echo $row['bookmark_name'] ; ?>?');" title="Edit" ><i class="fa fa-pencil"></i></a>
						<!---end edit page---->
						<!---status----->
						<?php  //if($row['video_status']==1){ ?>
						<!--<a href="<?php echo base_url() . "admin/bookmark/status/".$row['ID']."/0";?>" class="btn btn-success btn-xs"  title="Enable" onclick="return confirm('Are you sure you want to disable <?php $row['video_title']; ?>?');">
						<i class="fa fa-check"></i></a>
						<?php //} else { ?>
						<a class="btn btn-warning btn-xs" href="<?php echo base_url() . "admin/bookmark/status/".$row['ID']."/1";?>"  title="Disable" onclick="return confirm('Are you sure you want to enable <?php echo $row['video_title']; ?>?');"> 
						<i class="fa fa-check"></i></a>-->
						<?php //} ?> 
						
						<!----delete------>
						<a class="btn btn-danger btn-xs" href="<?php echo base_url() . "admin/bookmark/delete/".$row['ID'];  ?>" onclick="return confirm('Are you sure you want to remove this video?');" title="Remove"><i class="fa fa-trash-o "></i></a>
						<!---- end delete ------>
						</td>
                                  </tr>
				      <?php $i++; } ?>
