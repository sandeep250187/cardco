...
{
?>
<!--<h1><small>Your session will be expired in:-</small><span id="countdown" class="pull-right"> </span></h1> -->
  <div id="progress3">
                    <div class="row">
                        <div class="col-xs-9">
...
//$end = strtotime($data['cout']);
  ...
  //$night = ceil(abs($end - $start) / 86400);
  $result = array_unique($_SESSION['cart-hotel']);
	print_r($result);
	
// echo"pid=".$pid=$data['pid']; , SORT_REGULAR
...
<?php foreach($result as $val)
	{
		$pid2=$val['pid'];echo "</br>";
		
	
 echo"pid=".$pid2;//=$data['pid']; 
...
foreach($_SESSION['cart-hotel'] as $i=>$data) 
    {
		$pid=$data['pid']; 
		$sql2=mysql_query("select h.id,h.hotelname,h.hotel_pic,t.child_breakfast from mala_hotelmaster h ,apmg_hotel_tariff_withoutshtl t where h.id=t.hotel_id  and t.id='$pid'");  
  ...
  <h4 class="project-name">
    <?php
          ...
          if($data['room_type']==4){$rt="Family";}
			...
			$end = strtotime($data['cout']);
			$night = ceil(abs($end - $start) / 86400);
			$tot=($data['price']+$data['markup'])*$data['room']*$night; 
			$t=$tot+$t;
			$total=($tot+($row2['child_breakfast']*$data['child']));
			$child_breakfast+=($row2['child_breakfast']*$data['child']);
          ?>
  </h4>
  ...
  <div class="col-sm-12">
    <ul class="list-unstyled">
      <li>Until 23:59 PM on 31/03/2018 
        ...
        <span class="pull-center badge">MYR <?php echo number_format(((($t+$child_breakfast)*10)/100),2);  ?></span> </li>
      <li>After 23:59 PM on 31/03/2018 <span class="pull-center badge">MYR <?php echo number_format(((($t+$child_breakfast)*100)/100),2);  ?></span></li>
    </ul>
  </div>
  <h4>Remarks </h4>
...
<div class="panel-footer clearfix"> <a href="cart.php?id=<?php echo $i; ?>&act=remove"><i class="fa fa-times"></i> Delete Product</a> 
  ...
  <strong>Total MYR <?php echo  number_format(($t+$child_breakfast),2);
						$to=$to+$t+$child_breakfast;
						//echo  number_format($total+($row1['child_breakfast']*$data['child']),2); 
