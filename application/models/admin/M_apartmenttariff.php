<?php
class M_apartmenttariff extends CI_Model{

	
	function addapartmenttariff()
	{
		 
		$is_logged_in = $this->session->userdata('logged_in');
		  if($_FILES['room_pic1']['error']!= 4){
			$randno1 = rand();
			$string1 = str_replace(' ', '-',$_FILES['apt_pic1']['name']);
			$clearimg1 = preg_replace('/[^A-Za-z0-9\-.]/', '', $string1);
            $imagename1 = $randno1."_".$clearimg1;
             move_uploaded_file($_FILES['room_pic1']['tmp_name'],"uploads/apartmenttariff/".$imagename1);			
		}
		else {
			$imagename1='';
		}
		            ...
		            $config['source_image'] = "uploads/apartmenttariff/".$imagename1;
		...
		if($_FILES['room_pic2']['error']!= 4){
			$randno2 = rand();
			$string2 = str_replace(' ', '-',$_FILES['apt_pic2']['name']);
			$clearimg2 = preg_replace('/[^A-Za-z0-9\-.]/', '', $string2);
            $imagename2 = $randno2."_".$clearimg2;
             move_uploaded_file($_FILES['room_pic2']['tmp_name'],"uploads/apartmenttariff/".$imagename2);			
		}
		else {
			$imagename2='';
		}
		            ...
		            $config['image_library'] = 'gd2';
		            $config['allowed_types'] = 'gif|jpg|png|jpeg';
		            $config['source_image'] = "uploads/apartmenttariff/".$imagename2;
		 ...
		 $data = array(
		      'apt_id' => $this->input->post('aptid'),
			  'pic1'=>$imagename1,
			  'pic2'=>$imagename2,
			  'apt_type' => $this->input->post('apt_type'),
			  'apt_cat' => $this->input->post('apt_cat'),
			  'valid_from' => $this->input->post('valid_from'),
			  'valid_to' => $this->input->post('valid_to'),
			  'apt_price' => $this->input->post('apt_price'),
		...
		$msg = 0;
	    $this->db->insert('tbl_apartmenttariff', $data);
		...
		$number_of_files = sizeof($_FILES['image_file']['tmp_name']);
					 ...
					 if(sizeof($errors)==0)
					{
					  ...
					  for ($i = 0; $i < $number_of_files; $i++) {
						...
						move_uploaded_file($_FILES['image_file']['tmp_name'],"uploads/apartmentpicgallery/".$imgname);
					     /**Thumb Creation**/	
						$config['image_library'] = 'gd2';
						$config['allowed_types'] = 'gif|jpg|png|jpeg';
						$config['source_image'] = "uploads/apartmentpicgallery/".$imgname;
						...
						$this->image_lib->initialize($config);
						$this->image_lib->resize();
						$arraydaa = array('apt_id'=>$lastinsert_id,'image'=>$imgname);
						$this->M_apartmenttariff->uploadMultiplePic($arraydaa);
					  }
					}
				else
				{
	...
	function uploadMultiplePic($arraydaa){
		if(!empty($arraydaa)){
			$result = 0;
	        $this->db->insert('tbl_apartmentpicgallery', $arraydaa);
	 ...
	 function checkusername($username)
  {
   ...
   $this -> db -> from('tbl_apartmenttariff');
   ...
   { 
	...
	function index($id){
		$is_logged_in = $this->session->userdata('logged_in');
		 if($_FILES['pic1']['error']!= 4){
            ...
            $imagename1 = $randno."_".$clearimg;
             move_uploaded_file($_FILES['pic1']['tmp_name'],"uploads/apartmenttariff/".$imagename1);			
		}
		else {
			$imagename1=$this->input->post('apt_img1');
		}
		if($_FILES['pic2']['error']!= 4){
			...
			$imagename2=$this->input->post('apt_img2');;
		}
         $data = array(
	 ...
	 $msg = 0;
	    $this->db->where('id',$id);
	    $this->db->update('tbl_apartmenttariff', $data); 
	...
	function delete($pageid){
		...
		$this->db->delete('tbl_apartmenttariff', array('id' => $pageid));
		return ($this->db->affected_rows() != 1) ? false : true;
  	}
	function status($id,$status){
		$this->db->where('id',$id );
        $this->db->set("status", $status);
        $this->db->update("tbl_apartmenttariff");
	...
	function page_info($id){
		...
		/*$this->db->select('tbl_apartmenttariff.*,tbl_roompicgallery.image');
		  $this->db->from('tbl_apartmenttariff');
		  $this->db->join('tbl_roompicgallery','tbl_roompicgallery.hotel_id = tbl_apartmenttariff.id','left');
	      $this->db->where('tbl_apartmenttariff.id',$id);*/
