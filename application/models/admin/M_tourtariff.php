<?php
class M_tourtariff extends CI_Model{
	private $tbl_payment_criteria = 'tbl_pages';

	
	function addtourtariff()
	{
		$is_logged_in = $this->session->userdata('logged_in');		
		$data = array(
			'tid' => $this->input->post('tour_name'),
		...
		$this->db->insert('tbl_tourtariff_master', $data);
	...
	if( $id == 0 ){
		...
		$this->db->where('id',$id );
	    $this->db->update('tbl_tourtariff_master', $data); 
	...
	function delete($pageid){
		$this->db->delete('tbl_tourtariff_master', array('id' => $pageid));
		return ($this->db->affected_rows() != 1) ? false : true;
  	}
	function status($id,$status){
		$this->db->where('id',$id );
        $this->db->set("status", $status);
        $this->db->update("tbl_tourtariff_master");
