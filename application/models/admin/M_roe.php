<?php
class M_roe extends CI_Model{
	private $tbl_payment_criteria = 'tbl_roe_master';
    ...
    function index($id)
	{
		 ...
		 $msg = 0;
	    if( $id == 0 ){
			$this->db->insert('tbl_roe_master', $data);
			$msg = ($this->db->affected_rows() > 0) ? 1 : 0;//for insert
		}else{
			$this->db->where('id',$id );
	    	$this->db->update('tbl_roe_master', $data); 
	...
	function addroe()
	{
		...
		$roe_date= $this->input->post('roe_date');
		$this -> db -> select('roe_date');
        $this -> db -> from('tbl_roe_master');
        $this -> db -> where('roe_date', $roe_date);
        $query = $this->db->get();
	 ...
	 function checkusername($username)
  {
   ...
   { 
	...
	function delete($pageid){
	    ...
	    $link=base_url().'uploads/roe/'.$result['logo'];
	    unlink($link);
...
function Updatepriority($id,$priority)
 ...
 //extensive suitability check before doing anything with the file...
    ...
    else if (!is_uploaded_file($_FILES['upload']["tmp_name"]))
    {
      ...
      $url = $fdata[0]."uploads/roe/" . $time."_".$_FILES['upload']['name'];
    }
