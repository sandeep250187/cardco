<?php
class M_apartmentmaster extends CI_Model{
	private $tbl_payment_criteria = 'tbl_apartmentmaster';

	
	function addapartmentmaster()
	{
		$is_logged_in = $this->session->userdata('logged_in');
		 if($_FILES['apt_pic']['error']!= 4){
			$randno = rand();
			$string = str_replace(' ', '-',$_FILES['apt_pic']['name']);
			$clearimg = preg_replace('/[^A-Za-z0-9\-.]/', '', $string);
            $imagename = $randno."_".$clearimg;
             move_uploaded_file($_FILES['apt_pic']['tmp_name'],"uploads/apartmentmaster/".$imagename);			
		}
		else {
			$imagename='';
		}
		            ...
		            $config['source_image'] = "uploads/apartmentmaster/".$imagename;
		 ...
		 $data = array(
			  ...
			  'apt_map' => $this->input->post('maploc'),
			  'apt_description' => $this->input->post('apt_description')
		 );
		
		$msg = 0;
	    $this->db->insert('tbl_apartmentmaster', $data);
		...
		$number_of_files = sizeof($_FILES['image_file']['tmp_name']);
					 ...
					 if(sizeof($errors)==0)
					{
					  ...
					  for ($i = 0; $i < $number_of_files; $i++) {
						...
						$this->M_apartmentmaster->uploadMultiplePic($arraydaa);
					  }
					}
				else
				{
	 ...
	 function checkusername($username)
  {
   ...
   { 
	...
	{
		$this->db->select('tbl_apartmentmaster.*');
		$this->db->from('tbl_apartmentmaster');
		...
		$is_logged_in = $this->session->userdata('logged_in');
	      if($_FILES['apt_pic']['error']!= 4){
            ...
            $imagename = $randno."_".$clearimg;
			 ...
			 $imagename=$this->input->post('apt_img');
		}
         ...
         $data = array(
			 'aptname' => $this->input->post('aptName'),
			  ...
			  'apt_map' => $this->input->post('maploc'),
			  'apt_description' => $this->input->post('apt_description'),
			  'popular_apt' => $this->input->post('popular_apt'),
		 );
	...
	if( $id == 0 ){
		...
		$this->db->where('id',$id );
	    $this->db->update('tbl_apartmentmaster', $data); 
	...
	function delete($pageid){
		 $this->db->delete('tbl_apartmentmaster', array('id' => $pageid));
		return ($this->db->affected_rows() != 1) ? false : true;
  	}
	function status($id,$status){
		$this->db->where('id',$id );
        $this->db->set("status", $status);
        $this->db->update("tbl_apartmentmaster");
	...
	function page_info($id=0){
		$this->db->select('tbl_apartmentmaster.*,tbl_hotelpicgallery.image');
		$this->db->from('tbl_apartmentmaster');
		if(!empty($id)){
	      $this->db->join('tbl_hotelpicgallery','tbl_hotelpicgallery.hotel_id = tbl_apartmentmaster.id','left');
		  $this->db->where('tbl_apartmentmaster.id',$id);
		}
		$query = $this->db->get();
		return $query->result_array();
		//pr($result); 
