<?php
class M_apartmentcategory extends CI_Model{
	...
	function index($id){
		echo $id;
		 ...
		 $data = array(
		 	'aptcat' => $this->input->post('apartmentCategory'),
		 	'aptcap' => $this->input->post('aptcap'),
			
		 );
	 $msg = 0;
	if( $id == 0 ){
		$this->db->insert('tbl_aptcat_master', $data);
		$msg = ($this->db->affected_rows() > 0) ? 1 : 0;//for insert
	}else{
		$this->db->where('id',$id );
	    $this->db->update('tbl_aptcat_master', $data); 
	...
	function listing($limit,$start,$startdate,$enddate,$specificOption,$specificValue)
	{
	 ...
	 function count_all($startdate,$enddate,$specificOption,$specificValue){
		...
		$query =   $this->db->get('tbl_aptcat_master');
	...
	function delete($pageid){
	    ...
	    unlink($link);
	    $this->db->delete('tbl_aptcat_master', array('id' => $pageid));
