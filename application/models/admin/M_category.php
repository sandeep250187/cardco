...
class M_category extends CI_Model{
	private $tbl_payment_criteria = 'tbl_category';
	...
	function addcategory()
	{
		...
		//$escape=array("\'","\r\n");
		//$replace=array("'","");
		$text=str_replace("\'","'",$this->input->post('cat_desc'));
		$text1=str_replace("\'","'",$this->input->post('cat_sub_desc'));
		...
		if($_FILES['file_name']['error']!= 4){
            ...
            $imgname = $randno."_".$clearimg;
             move_uploaded_file($_FILES['file_name']['tmp_name'],"uploads/category/".$imgname);			
		}
		else {
			$imgname='';
		}
	...
	function index($id){
		...
		//$replace=array("'","");
		$text=str_replace("\'","'",$this->input->post('cat_desc'));
		$text=str_replace($replacestring,"",$this->input->post('cat_desc'));
		
		$text1=str_replace($replacestring,"",$this->input->post('cat_sub_desc'));
		//$content=mysql_real_escape_string($text);
		...
		if($_FILES['file_name']['error']!= 4){
			...
			if($this->input->post('h_img')!=''){
				$linlk = base_url().'uploads/category/'.$this->input->post('h_img');
				unlink($linlk);
			}
             move_uploaded_file($_FILES['file_name']['tmp_name'],"uploads/category/".$imgname);			
		}
	...
	function delete($pageid){
		$this->db->delete('tbl_category', array('id' => $pageid));
...
function Updatepriority($id,$priority)
 ...
 //extensive suitability check before doing anything with the file...
    ...
    else if (!is_uploaded_file($_FILES['upload']["tmp_name"]))
    {
      ...
      $url = $fdata[0]."uploads/category/" . $time."_".$_FILES['upload']['name'];
    }
