<?php
class M_tourmaster extends CI_Model{
	...
	function addtour()
	{
		$is_logged_in = $this->session->userdata('logged_in');
		 if($_FILES['tour_image']['error']!= 4){
			$randno = rand();
			$string = str_replace(' ', '-',$_FILES['tour_image']['name']);
			$clearimg = preg_replace('/[^A-Za-z0-9\-.]/', '', $string);
            $imgname = $randno."_".$clearimg;
             move_uploaded_file($_FILES['tour_image']['tmp_name'],"uploads/tourname/".$imgname);			
		}
		else {
			$imgname='';
		}
		            ...
		            $config['source_image'] = "uploads/tourname/".$imgname;
		...
		$data = array(
			...
			'pax1' => $this->input->post('pax1'),
			'pax4' => $this->input->post('pax4'),
			'pax9' => $this->input->post('pax9'),
			'pax16' => $this->input->post('pax16'),
			'pax25' => $this->input->post('pax25'),
			'pax31' => $this->input->post('pax31'),
			'pax40' => $this->input->post('pax40'),
		...
		$this->db->insert('tbl_penang_tour_master', $data);
	...
	function index($id){
		...
		$data = array(
			...
			'validity' => $this->input->post('validity'),
			'description' => $this->input->post('description'),
			'popular_tour_pkg' => $this->input->post('popular_tour_pkg'),
			'thumbnail'=>$imgname,
		);
	...
	if( $id == 0 ){
		...
		$this->db->where('id',$id);
	    $this->db->update('tbl_penang_tour_master', $data); 
	...
	function delete($pageid){
		$this->db->delete('tbl_penang_tour_master', array('id' => $pageid));
		return ($this->db->affected_rows() != 1) ? false : true;
  	}
	function status($id,$status){
		$this->db->where('id',$id );
        $this->db->set("status", $status);
        $this->db->update("tbl_penang_tour_master");
