...
class M_banner extends CI_Model{
	private $tbl_payment_criteria = 'tbl_banner';
	...
	function addbanner()
	{
		...
		if($_FILES['image_path']['error']!= 4){
            ...
            $imgname = $randno."_".$clearimg;
             move_uploaded_file($_FILES['image_path']['tmp_name'],"uploads/banner/".$imgname);			
		}
		else {
			$imgname='';
		}
		...
		$data = array('title' => $this->input->post('title'),
			'content' => $text,
			'image_path'=>$imgname,
			//'page_slug' => strtolower($this->input->post('page_slug')),
	...
	function index($id){
		...
		if($_FILES['image_path']['error']!= 4){
			...
			if($this->input->post('h_img')!=''){
				$linlk = base_url().'uploads/banner/'.$this->input->post('h_img');
				unlink($linlk);
			}
             move_uploaded_file($_FILES['image_path']['tmp_name'],"uploads/banner/".$imgname);			
		}
	...
	function delete($pageid){
		$this->db->delete('tbl_banner', array('id' => $pageid));
...
function Updatepriority($id,$priority)
 ...
 //extensive suitability check before doing anything with the file...
    ...
    else if (!is_uploaded_file($_FILES['upload']["tmp_name"]))
    {
      ...
      $url = $fdata[0]."uploads/banner/" . $time."_".$_FILES['upload']['name'];
    }
